...
def load_features_and_targets(dataset_path):
    features_per_day = {}
    targets_per_day = {}
    for file in os.listdir(dataset_path):
        day = file.split("_")[0]
        values = np.load(dataset_path + file)
        if "targets" in file:
            targets_per_day[day] = values
        elif "features" in file:
            features_per_day[day] = values
        else:
            raise NotImplementedError()

    for day in features_per_day:
        assert len(targets_per_day[day]) <= len(features_per_day[day])
        features_per_day[day] = features_per_day[day][: len(targets_per_day[day])]
